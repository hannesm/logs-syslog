...
(** UDP syslog *)
module Udp (CLOCK : Mirage_clock.PCLOCK) (STACK : Mirage_stack.V4) : sig
  (** [create udp ~hostname ip ~port ~truncate ()] is [reporter], which
      sends log messages to [ip, port] via UDP.  Upon failure, a message is
      emitted using Printf.  Each message can be truncated: [truncate]
...
(** TCP syslog *)
module Tcp (CLOCK : Mirage_clock.PCLOCK) (STACK : Mirage_stack.V4) : sig
  (** [create tcp ~hostname ip ~port ~truncate ~framing ()] is
      ...
      emitted using Printf, and attempts are made to re-establish the TCP
