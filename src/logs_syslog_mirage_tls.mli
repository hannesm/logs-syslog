...
module Tls (C : V1.CLOCK) (TCP : V1_LWT.TCP) (KV : V1_LWT.KV_RO) : sig
  ...
  (** [create clock tcp kv ~hostname ip ~port ~framing ()] is [Ok reporter] or
      [Error msg].  The [reporter] sends log messages to [ip, port] via TLS.  If
      ...
      message.  The [port] defaults to 6514, [framing] to prepend the length in
      decimal.  *)
  val create : TCP.t -> KV.t -> hostname:string -> TCP.ipaddr -> ?port:int ->
