...
module Tls (C : V1.PCLOCK) (TCP : V1_LWT.TCP) (KV : V1_LWT.KV_RO) : sig
  ...
  (** [create clock tcp kv ~hostname ip ~port ()] is [Ok reporter] or [Error
      ...
      message.  The [port] defaults to 6514. *)
  val create : C.t -> TCP.t -> KV.t -> hostname:string -> TCP.ipaddr -> ?port:int -> unit ->
