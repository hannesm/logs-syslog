...
let message ?(facility = Syslog_message.System_Daemons) ~host ~source ~tags ?header level timestamp message =
  ...
  let message = Printf.sprintf "%s%s%s %s" source tags hdr message in
