...
(** [tcp_tls_reporter ~hostname remote_ip ~port ~cacert ~cn ~cert ~priv_key
    ~framing ()] is [Ok reporter] or [Error msg].  The [reporter] sends each log
    message to [remote_ip, port] via TLS.  If the initial TLS connection to the
    ...
    {{:https://tools.ietf.org/html/rfc5280}RFC 5280}).  [cn] must be the common
    name of the certificate of the communication partner.  The default value for
